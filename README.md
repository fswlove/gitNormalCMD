...
## git push < 服务器名，如origin > --all  

将本地所有分支推送到远程仓库  

## git push -u < 服务器名，如origin > --all  

将本地所有分支推送到远程仓库，并且将分支关联远程仓库  
...
## git commit --amend 后无法推送到远程仓库  

与远程仓库有分歧  

git push --force-with-lease <服务器名> <分支名> 或者 git push -f  

**`--force-with-lease` 和 `-f (--force)` 的区别：**  

- `--force`：**无条件**地用你的本地分支覆盖远程分支。如果其他人在你之后推送了代码，他们的提交会被悄无声息地覆盖丢失，非常危险。  
- `--force-with-lease`：**更安全的选择**。它会检查远程分支的最新状态是否和你上次拉取时一样。如果期间有其他人推送了代码，这个命令会失败，提醒你先合并他们的工作。这可以防止你意外覆盖同事的代码。  
